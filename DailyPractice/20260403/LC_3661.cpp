...
public:
    ...
    int maxWalls(vector<int>& robots, vector<int>& distance, vector<int>& walls) {
        ...
        for(int i=0;i<n;i++){
            int after = rD[i].first + rD[i].second;
            int before0 = rD[i].first - rD[i].second;
            int before1 = before0;
            if (i != n-1) {
                after = min(after,rD[i+1].first-1);
            }

            if(i != 0){
                before0 = max(before0,rD[i-1].first+1);//when previous is to left side
                before1 = min(rD[i].first, max(before0,rD[i-1].first+ rD[i-1].second+1));//when previous is to right
            }
